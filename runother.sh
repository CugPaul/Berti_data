...
TRACES_SPEC="traces/gap"
OUT_SPECname=$(basename "$TRACES_SPEC")
...
file_trace () 
{
    ...
    # for trace in "${spec2k17_one[@]}";  #modified
    # do
    #     i="$2/$trace"
    #     trace=$(basename "$i")
...
while getopts :mfvlrcdhnegp: opt; do
    case "${opt}" in
          v) VERBOSE="Y"
          ...
          n) BUILD="N"
              ...
              ;;
          e) EXE="N"
              echo -e "\033[1mNOT EXE the simulator\033[0m"
              ;;
          f) FULL="Y"
...
if [[ "$BUILD" == "Y" ]]; then
    ...
    # cd $BERTI
    # run_compile "./build_champsim.sh hashed_perceptron no perceptron no no no no no\
