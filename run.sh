...
TRACES_SPEC="traces/spec_prefetchTest"
OUTPUT_SPEC="traces/spec2k17_all_delta_613"
OUT_SPECname=$(basename "$OUTPUT_SPEC")
...
file_trace () 
{
    for trace in "${spec2k17_select[@]}";  #modified
    do
        i="$2/$trace"
    ...
    # do
        ...
        trace=$(basename "$i")
        trace=${trace#*.}                   # 去掉前缀到第一个点（即 "605."）
        trace=${trace%.champsimtrace.xz}    # 去掉后缀

        if [[ "$LOGGED" == "Y" ]]; then
            echo -n "$1 -warmup_instructions 50000000 -simulation_instructions"
            echo " 200000000 -traces $i > $OUT/$3-$trace.xz 2>>$LOG"
        else
            echo -n "$1 -warmup_instructions 50000000 -simulation_instructions"
            echo " 200000000 -traces $i > $OUT/$3-$trace.xz 2>/dev/null"
        fi
    done
}
...
if [[ "$BUILD" == "Y" ]]; then
    ...
    echo -n "Building Perceptron..."
    cd $BERTI
    run_compile "./buildperceptron_champsim.sh hashed_perceptron no perceptron no no no no no\
            lru lru lru srrip drrip lru lru lru 1 no 16 4 $NAME"  #打引号表示一个整体传入
...
if [[ "$VERBOSE" == "Y" ]]; then
    python3 Python/get_data.py y output/$OUT_SPECname > single.csv
elif [[ "$LOGGED" == "Y" ]]; then
    python3 Python/get_data.py y output/$OUT_SPECname > single.csv 2>>$LOG
else
    python3 Python/get_data.py y output/$OUT_SPECname > single.csv 2>/dev/null
fi
echo " ${GREEN}done${NC}"
...
if [[ "$VERBOSE" == "Y" ]]; then
    python3 Python/get_data_fig.py y output/$OUT_SPECname >> single.csv
elif [[ "$LOGGED" == "Y" ]]; then
    python3 Python/get_data_fig.py y output/$OUT_SPECname >> single.csv 2>>$LOG
else
    python3 Python/get_data_fig.py y output/$OUT_SPECname >> single.csv 2>/dev/null
fi
run_command "python3 Python/one_prefetch_performance.py single.csv"

echo -n "Generating Figure 9 (a)..."
if [[ "$VERBOSE" == "Y" ]]; then
    python3 Python/get_data_by_traces.py y SpeedUp output/$OUT_SPECname > spec.csv
elif [[ "$LOGGED" == "Y" ]]; then
    python3 Python/get_data_by_traces.py y SpeedUp output/$OUT_SPECname > spec.csv 2>>$LOG
else
    python3 Python/get_data_by_traces.py y SpeedUp output/$OUT_SPECname > spec.csv 2>/dev/null
fi
